...
if (config.run_mode == "training"):
    ...
    dataprocess.get_words_idtif(dataframe)
    exit
    print("Feature Start")
    ...
    ent_y_title_len = np.array(list(map(lambda x:len(x),dataframe["title"].values)))
    ...
    if (config.model_type == "svc"):
        createModels.get_svc_model(X,y)
    else:    
        createModels.get_keras_cnn_model(X,y)
else:
    # 读取测试集
